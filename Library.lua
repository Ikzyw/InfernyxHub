...
local Library = {
    ...
    Scheme = {
        ...
        --// Granular overrides \\--
        TabColor    = Color3.fromRGB(25, 25, 25),   -- sidebar tab button bg
        SliderColor = Color3.fromRGB(125, 85, 255), -- slider fill bar
...
local VitaThemes = {
    ...
    Purple = {
        BackgroundColor = Color3.fromRGB(16, 10, 26),
        MainColor       = Color3.fromRGB(26, 16, 42),
        AccentColor     = Color3.fromRGB(155, 55, 255),
        OutlineColor    = Color3.fromRGB(58, 32, 88),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Blue = {
        ...
        AccentColor     = Color3.fromRGB(35, 120, 255),
        OutlineColor    = Color3.fromRGB(28, 48, 85),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Cyan = {
        BackgroundColor = Color3.fromRGB(7, 17, 20),
        MainColor       = Color3.fromRGB(10, 26, 32),
        AccentColor     = Color3.fromRGB(0, 200, 222),
        OutlineColor    = Color3.fromRGB(18, 55, 68),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Green = {
        BackgroundColor = Color3.fromRGB(8, 16, 10),
        MainColor       = Color3.fromRGB(12, 24, 16),
        AccentColor     = Color3.fromRGB(40, 210, 90),
        OutlineColor    = Color3.fromRGB(22, 54, 28),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Red = {
        BackgroundColor = Color3.fromRGB(18, 8, 8),
        MainColor       = Color3.fromRGB(28, 14, 14),
        AccentColor     = Color3.fromRGB(220, 45, 45),
        OutlineColor    = Color3.fromRGB(62, 22, 22),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Orange = {
        BackgroundColor = Color3.fromRGB(18, 12, 6),
        MainColor       = Color3.fromRGB(28, 18, 8),
        AccentColor     = Color3.fromRGB(255, 145, 30),
        OutlineColor    = Color3.fromRGB(65, 40, 14),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Pink = {
        BackgroundColor = Color3.fromRGB(20, 8, 16),
        MainColor       = Color3.fromRGB(32, 14, 26),
        AccentColor     = Color3.fromRGB(255, 75, 160),
        OutlineColor    = Color3.fromRGB(65, 22, 52),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Light = {
        BackgroundColor = Color3.fromRGB(228, 228, 232),
        MainColor       = Color3.fromRGB(210, 210, 216),
        AccentColor     = Color3.fromRGB(100, 58, 220),
        OutlineColor    = Color3.fromRGB(180, 180, 186),
        FontColor       = Color3.fromRGB(20, 20, 22),
        IsLight         = true,
    },
}

Library.Themes = VitaThemes
...
--// SetTheme: apply a named theme preset
function Library:SetTheme(Name: string)
    local Theme = VitaThemes[Name]
    if not Theme then
        local names = {}
        for k in VitaThemes do table.insert(names, k) end
        table.sort(names)
        warn(string.format("[VitaLib] Unknown theme %q | Available: %s", Name, table.concat(names, ", ")))
        return
    end
    ...
    -- Sync granular colors to theme defaults (only if user hasn't manually overridden them)
    Library.Scheme.TabColor    = Theme.MainColor    or Library.Scheme.MainColor
    Library.Scheme.SliderColor = Theme.AccentColor  or Library.Scheme.AccentColor
    Library.IsLightTheme = Theme.IsLight or false
    Library:UpdateColorsUsingRegistry()
    getgenv().VitaCurrentTheme = Name
end

--// UpdateScheme: partially override specific Scheme keys
function Library:UpdateScheme(Partial: {[string]: any})
    assert(typeof(Partial) == "table", "[VitaLib] UpdateScheme expects a table")
...
--// AddTheme: register a custom theme
function Library:AddTheme(Name: string, Theme: {[string]: Color3})
    assert(typeof(Name) == "string", "[VitaLib] Theme name must be a string")
    assert(typeof(Theme) == "table",  "[VitaLib] Theme must be a table")
    VitaThemes[Name] = Theme
end

--// GetThemeNames: return all registered theme names sorted
function Library:GetThemeNames(): {string}
    local Names = {}
    for Name in VitaThemes do table.insert(Names, Name) end
    table.sort(Names)
    return Names
end

--// SetAccent / SetBackground: convenience one-liners
function Library:SetAccent(Color: Color3)
    Library:UpdateScheme({ AccentColor = Color })
end

function Library:SetBackground(Color: Color3)
    Library:UpdateScheme({ BackgroundColor = Color })
end

--// FlashAccent: temporarily swap accent then revert
function Library:FlashAccent(NewColor: Color3, Duration: number?)
    local Original = Library.Scheme.AccentColor
    Library:UpdateScheme({ AccentColor = NewColor })
    task.delay(Duration or 0.6, function()
        Library:UpdateScheme({ AccentColor = Original })
    end)
end
...
--//  Example:  Library:Color("#ff5500")                         \\--
--//─────────────────────────────────────────────────────────────\\--
function Library:Color(Hex: string): Color3
    Hex = Hex:gsub("#", ""):gsub("0x", "")
    assert(#Hex == 6, string.format("[VitaLib] Color(): invalid hex %q (need 6 chars)", Hex))
...
--//─────────────────────────────────────────────────────────────\\--
--//  Library:SetColors({ Main, Tab, Slider, ... })              \\--
--//  Granular color control with hex strings OR Color3 values.  \\--
--//                                                             \\--
--//  Keys:                                                       \\--
--//    Main       → MainColor  (buttons, groupbox BG, bar BG)   \\--
--//    Tab        → TabColor   (sidebar tab button background)   \\--
--//    Slider     → SliderColor (slider fill / progress bar)     \\--
--//    Accent     → AccentColor (general accent / toggle ON)     \\--
--//    Background → BackgroundColor (window + container BG)      \\--
--//    Outline    → OutlineColor (borders / strokes)             \\--
--//    Font       → FontColor  (all text)                        \\--
--//─────────────────────────────────────────────────────────────\\--
function Library:SetColors(Colors: {[string]: any})
    assert(typeof(Colors) == "table", "[VitaLib] SetColors expects a table")
    ...
    local Patch = {}
    for ShortKey, Value in Colors do
        local SchemeKey = KeyMap[ShortKey] or ShortKey
        if typeof(Value) == "string" then
            Patch[SchemeKey] = Library:Color(Value)
        elseif typeof(Value) == "Color3" then
            Patch[SchemeKey] = Value
        end
    end

    Library:UpdateScheme(Patch)
end

--// SetStyle: change the global corner rounding
--   Options: "Square" (4px), "Curved" (12px), "Round" (18px), "Pill" (28px)
function Library:SetStyle(StyleName: string)
    local Radius = VitaStyleMap[StyleName]
    if not Radius then
        warn(string.format("[VitaLib] Unknown style %q | Options: Square, Curved, Round, Pill", StyleName))
        return
    end
    local OldHalf = math.floor(Library.CornerRadius / 2)
    local NewHalf = math.floor(Radius / 2)
    for _, Corner in Library.Corners do
        if Corner.CornerRadius.Offset == OldHalf then
            Corner.CornerRadius = UDim.new(0, NewHalf)
        else
            Corner.CornerRadius = UDim.new(0, Radius)
        end
    end
    Library.CornerRadius = Radius
    getgenv().VitaCurrentStyle = StyleName
end

function Library:GetCurrentStyle(): string
    return getgenv().VitaCurrentStyle or "Curved"
end

--// Global Color() shorthand — usable anywhere as Color("FF5500") or Color("#FF5500")
getgenv().Color = function(Hex: string): Color3
    return Library:Color(Hex)
end
...
function Library:CreateWindow(WindowInfo)
    ...
    local MinimizeBtn
    local MinimizeIcon
    local _VitaMinimized  = false
    local _VitaFullHeight = nil
    local _DoMinimize     = nil  -- set below inside do block
    ...
    do
        ...
        RightWrapper = New("Frame", {
            ...
            Size = UDim2.new(1, -InitialLeftWidth - 100 - 1, 1, -16),
            Parent = TopBar,
        })
        ...
        --// Minimize Button — top-right corner of TopBar \\--
        do
            local MinusIcon  = Library:GetIcon("minus")
            local MaxIcon    = Library:GetIcon("maximize-2")
            local MinBtnSize = 28

            local MinHolder = New("Frame", {
                AnchorPoint      = Vector2.new(1, 0.5),
                BackgroundColor3 = "MainColor",
                Position         = UDim2.new(1, -10, 0.5, 0),
                Size             = UDim2.fromOffset(MinBtnSize, MinBtnSize),
                ZIndex           = 50,
                Parent           = TopBar,
            })
            table.insert(Library.Corners, New("UICorner", {
                CornerRadius = UDim.new(0, math.floor(Library.CornerRadius / 2)),
                Parent       = MinHolder,
            }))
            New("UIStroke", {
                Color     = "OutlineColor",
            ...
            Library:AddToRegistry(MinHolder, { BackgroundColor3 = "MainColor" })
            ...
            MinimizeIcon = New("ImageLabel", {
                AnchorPoint            = Vector2.new(0.5, 0.5),
                BackgroundTransparency = 1,
                Image                  = MinusIcon and MinusIcon.Url or "",
                ImageColor3            = "FontColor",
                ImageRectOffset        = MinusIcon and MinusIcon.ImageRectOffset or Vector2.zero,
                ImageRectSize          = MinusIcon and MinusIcon.ImageRectSize  or Vector2.zero,
            ...
            Library:AddToRegistry(MinimizeIcon, { ImageColor3 = "FontColor" })

            local function DoMinimize(State: boolean)
                _DoMinimize = _DoMinimize  -- forward ref already set
                _VitaMinimized = State
                MainFrame.ClipsDescendants = true

                if State then
                    _VitaFullHeight = MainFrame.Size.Y.Offset
                    if MaxIcon then
                        MinimizeIcon.Image           = MaxIcon.Url
                        MinimizeIcon.ImageRectOffset = MaxIcon.ImageRectOffset
                        MinimizeIcon.ImageRectSize   = MaxIcon.ImageRectSize
                    end
                else
                    if MinusIcon then
                        MinimizeIcon.Image           = MinusIcon.Url
                        MinimizeIcon.ImageRectOffset = MinusIcon.ImageRectOffset
                        MinimizeIcon.ImageRectSize   = MinusIcon.ImageRectSize
                    end
                end

                -- Hide every MainFrame child that is NOT the TopBar (48px frame)
                for _, Child in MainFrame:GetChildren() do
                    if Child:IsA("GuiObject") then
                        local isTopBar = Child:IsA("Frame")
                            and Child.Size.Y.Offset == 48
                ...
                -- Tween height: 49px (just the top bar) or full height
                local TargetH = State and 49 or (_VitaFullHeight or 600)
                TweenService:Create(MainFrame,
                    TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
                    { Size = UDim2.new(MainFrame.Size.X.Scale, MainFrame.Size.X.Offset, 0, TargetH) }
                ):Play()

                getgenv().VitaMinimized = State
            end
            _DoMinimize = DoMinimize

            MinimizeBtn.MouseButton1Click:Connect(function()
                DoMinimize(not _VitaMinimized)
            end)

            MinimizeBtn.MouseEnter:Connect(function()
                TweenService:Create(MinHolder, Library.TweenInfo, {
            ...
            -- Restore minimized state across re-executes
            if getgenv().VitaMinimized then
                task.defer(function()
                    _VitaFullHeight = MainFrame.Size.Y.Offset
                    DoMinimize(true)
                end)
    ...
    --// Window:Minimize / Restore / ToggleMinimize / IsMinimized \\--
    function Window:Minimize()
        if not _VitaMinimized and _DoMinimize then
            _DoMinimize(true)
        end
    end

    function Window:Restore()
        if _VitaMinimized and _DoMinimize then
            _DoMinimize(false)
        end
    end
    ...
    function Window:ToggleMinimize()
        if _DoMinimize then
            _DoMinimize(not _VitaMinimized)
        end
    end

    function Window:IsMinimized(): boolean
        return _VitaMinimized
    end

    --// Window:SetTabHidden(tabName, hidden) \\--
    function Window:SetTabHidden(TabName: string, Hidden: boolean)
        local Tab = Library.Tabs[TabName]
        if Tab and Tab.SetHidden then
            Tab:SetHidden(Hidden)
        else
            warn(string.format("[VitaLib] SetTabHidden: tab %q not found", tostring(TabName)))
        end
    end
    ...
    function Window:SetSidebarWidth(Width)
        ...
        RightWrapper.Size = UDim2.new(1, -Width - 100 - 1, 1, -16)
    ...
    function Window:AddTab(...)
        ...
        --// Tab:SetHidden — hide or reveal this tab's sidebar button \\--
        local _TabSidebarHidden = false
        Tab._SidebarHidden = false

        function Tab:SetHidden(Hidden: boolean)
            _TabSidebarHidden = Hidden
            Tab._SidebarHidden = Hidden
            TabButton.Visible = not Hidden

            if Hidden and Library.ActiveTab == Tab then
                -- Auto-switch to the next visible tab
                for _, OtherTab in Library.Tabs do
                    if typeof(OtherTab) == "table"
                        and OtherTab ~= Tab
                        and not OtherTab._SidebarHidden
                    then
                        OtherTab:Show()
                        return
                    end
        ...
        function Tab:IsHidden(): boolean
            return _TabSidebarHidden
        end

        function Tab:ToggleHidden()
            Tab:SetHidden(not _TabSidebarHidden)
        end
...
getgenv().VitaThemes = Library.Themes
return Library
